                ...
                #' @param xlab label or expression for x-axis. Default: NULL results in "log2FC" 
                #' @param ylab label or expression for x-axis. Default: NULL results in "log2FC" 
                #' @param max.overlaps Integer. Max overlapping labels (ggrepel). Default: 10.
                #' @param min.segment.length Integer, Minimum segment length. Default: 0.5 (Units line)
                #' @param pval_colname Character. Column name for raw p-values. e.g. arrow(length = unit(0.015, "npc"). Default: "PValue".
                #' @param adjpval_colname Character. Column name for adjusted p-values. Default: "FDR".
                #' @param fold_change_colname Character. Column name for logFC. Default: "logFC".
                ...
                volcano_plot <- function(top,
                  ...
                  required_cols <- c(fold_change_colname, pval_colname, adjpval_colname, label_column)
                  missing_cols <- setdiff(required_cols, colnames(top))
                  if (length(missing_cols) > 0) {
                    stop("Missing column(s): ", paste(missing_cols, collapse = ", "),
                         "\nCheck input or adjust: pval_colname, adjpval_colname, fold_change_colname, label_column")
                  }
                  ...
                  top <- top %>%
                    dplyr::select(all_of(c(fold_change_colname, pval_colname, adjpval_colname, label_column)))
                  if (fold_change_colname != "logFC") top <- top %>% rename(logFC = !!sym(fold_change_colname))
