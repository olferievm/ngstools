...
glucocorticoid_genes <- function(x, controls = 1) {
  requireNamespace("edgeR", quietly = TRUE)
  
  # Define the 21 interferon-stimulated genes
  gluc_genes <- c( "ECHDC3", "ZBTB16", "DUSP1", "FAR2",
                  "HDAC4", "GRB10", "ERLIN1",
                  "RNASE1", "ATP8B4", "FKBP5",
                  "FZD5", "TEX2", "CAMKK2",
                  "DAAM2", "B4GALT5", "SPTLC2"
                  )
  ...
  if (inherits(x, "DGEList")) {
    ...
    matched_genes <- intersect(x$genes$gene_name, gluc_genes)
    
    if (length(matched_genes) == 0) stop("No interferon genes found. Abort.")
    if (length(matched_genes) < 4) stop("Only ", length(matched_genes), " genes found. Abort.")
    if (length(matched_genes) < 8) warning("NOTE: Found ", length(matched_genes), " genes.")
